...
namespace livemap {
    ...
    /*!
     ...
     @details server_base 클래스에서 연결된 소켓을 받아 서비스를 제공하는 인터페이스 제공.
    ...
    public:
        ...
        /*!
         ...
         @details server_base 클래스의 set_delegate 함수 안에서 호출된다.
        ...
        /*!
         ...
         @details 클라이언트에서 넘겨받은 소켓 타입은 void 포인터다. 소켓타입은 server_service_delegate
        ...
        /*!
         ...
         @details 에러가 발생했을때 오류가 발생한 소켓과 에러 정보가 같이 전달된다. 소켓객체와 에러객체는 이 메서드에서 수명이 끝나야 한다.
    ...
    /*!
     @breif 서버 베이스 클래스
     @details 서버 베이스 클래스는 모든 종류의 서버 클래스는 이 클래스를 상속한 클래스다.
    ...
    public:
        ...
        /*
         ...
         @details 세션빌더 타입의 세션 빌더 함수 객체는 인자로 소켓 객체를 입력받고, 세션 객체를 생성하여 반환한다.
         세션빌더는 세션 팩토리 함수이다.
         */
        ...
        /*!
         ...
         @details 이 클래스를 상속하는 클래스에서는 클라이언트로부터 오는 연결요청을 받아들이는 작업을 시작하도록 구현해야한다.
         */
        virtual void start_service() = 0;
        /*!
         @breif 클라이언트 연결 종료 인터페이스.
         @details 이 클래스를 상속하는 클래스에서는 클라이언트로부터 오는 연결요청을 받아들이는 작업을 종료하도록 구현해야한다.
         */
        virtual void stop_service() = 0;
        /*!
         @breif 세션 빌더 함수 객체를 반환하는 인터페이스.
         @details session_builder_type형 함수객체를 반환한다 이 함수객체에는 세션을 생성할 수 있는 구현이 포함되어 있다.
         */
        ...
        /*!
         ...
         @details 이 함수를 통해 server_service_delegate 계열 객체를 설정하게 되면
        ...
        /*!
         ...
         @details server_service_delegate 계열 클래스 객체 포인터를 반환해도 server_base 계열 포인터가
    ...
    private:
        ...
        /*!
         ...
         @details server_service_delegate 계열 클래스 객체 포인터를 std::unique_ptr가 감싸고 있다.
